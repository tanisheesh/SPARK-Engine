...
function ChenDiagramContent({ schema, graph }: any) {
        ...
        tables.forEach((table) => {
            ...
            attrs.forEach((col: string, j: number) => {
                ...
                newEdges.push({
                    ...
                    target: attrId,
                    type: "waypoint",
                    style: { stroke: "#70716C", strokeWidth: 1 }
                });
            });
        });
        ...
        Object.keys(graph || {}).forEach((from, i) => {
            graph[from]?.forEach((rel: any, j: number) => {
                ...
                newEdges.push({
                    ...
                    labelStyle: { fill: "#C8FF3D", fontWeight: "900", fontSize: 14 },
                    style: { stroke: "#C8FF3D", strokeWidth: 2 }
                });
    ...
    const onConnect = useCallback(
        (params: Connection) =>
            setEdges(eds => addEdge({ ...params, type: "waypoint", style: { stroke: "#C8FF3D", strokeWidth: 2 }, deletable: true }, eds)),
        [setEdges]
    );
    ...
    return (
        <div className="w-full h-full bg-bg relative">
            <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex gap-2 bg-surface2 border border-line rounded-2xl px-4 py-2">
                <button
                    onClick={() => setEditMode(m => !m)}
                    className={`px-3 py-1.5 rounded-md text-sm transition-colors duration-1 ${editMode ? "bg-accent-soft text-accent" : "text-muted hover:text-ink hover:bg-surface3"}`}
                >
                    {editMode ? "Editing" : "Edit"}
                </button>
                {editMode && (
                    <span className="text-[10px] text-faint self-center">Drag handle to handle to connect · Select edge + Delete to remove</span>
                )}
                
                <button
                    onClick={downloadImage}
                    className="px-3 py-1.5 rounded-xl text-[11px] transition-colors duration-1 text-muted hover:text-accent hover:bg-surface3 flex items-center gap-2"
                >
            ...
            >
                <Background color="#272828" gap={20} />
                <Controls />
            </ReactFlow>
        </div>
    );
