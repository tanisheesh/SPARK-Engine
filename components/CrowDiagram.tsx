...
function CrowDiagramContent({ schema, graph }: CrowDiagramProps) {
    ...
    Object.keys(graph || {}).forEach((from) => {
      (graph[from] || []).forEach((rel: any) => {
        ...
        edges.push({
          ...
          labelBgStyle: { fill: "#1E1F1F", fillOpacity: 0.9 },
          style: { stroke: "#70716C", strokeWidth: 1.5 },
          animated: false,
  ...
  const layoutButtons: { label: string; mode: LayoutMode; ns: number }[] = [
    { label: "Left to right", mode: "LR", ns: 140 },
    { label: "Top down", mode: "TB", ns: 140 },
    { label: "Compact", mode: "LR", ns: 50 },
      ...
      <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex gap-2 bg-surface2 border border-line rounded-2xl px-4 py-2">
        {layoutButtons.map(({ label, mode, ns }) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[11px] transition-colors duration-1 ${layoutMode === mode && currentNs === ns ? "bg-accent-soft text-accent border border-accent-line" : "text-muted hover:text-accent hover:bg-surface3"}`}
          >
            {label}
          </button>
        ))}
