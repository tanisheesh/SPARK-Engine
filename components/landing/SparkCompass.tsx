...
function Particle({ tx, ty, delay, dur, sz, scale }: {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: dur, repeat: Infinity, delay, ease: [0.2, 0, 0.8, 1] as const }}
    />
  );
}
...
export default function SparkCompass({
  ...
  useEffect(() => {
    ...
    async function sequence() {
      await cardinalCtrl.start({ pathLength: 1, opacity: 0.9, transition: { duration: 1.2, ease: [0.16, 1, 0.3, 1] as const } });
      diagCtrl.start({ pathLength: 1, opacity: 0.45, transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1] as const } });
        ...
        <div key={pulse.id} style={{ position: 'absolute', top: pulse.y, left: pulse.x, pointerEvents: 'none' }}>
          {[0, 1, 2, 3].map(i => (
            <motion.div key={i}
              ...
              transition={{ duration: 1.0, delay: i * 0.1, ease: [0.2, 0, 0.6, 1] as const }}
            />
          ))}
